...
class Parser:
    ...
    def process(self):
        ...
        """
        trial_dirs = [d for d in listdir(self.config_dir) if isdir(join(self.config_dir, d))]
    ...
    def aggregate_trials(self, trial_dirs):
        ...
        trial_output_dirs = [join(self.config_dir, d, 'output') for d in trial_dirs]

        csv_files = [f for f in listdir(trial_output_dirs[0]) if f.endswith('.csv')]

        for file in csv_files:
            dfs = []
            for trial in trial_output_dirs:
                path = join(trial, file)
            ...
            if any(metric in file for metric in ['rsrp', 'rsrq', 'rssi']):
                vstack_df = pd.concat(aligned_dfs, axis=0, ignore_index=True)
                vstack_df.to_csv(join(output_path, f'vstack_{file}'), index=False)
    ...
    def parse_rfsts(path, output_dir):
        ...
        results = {}
        for file in files:
            cid = re.search(r'rfsts.csv_\d+_CID(\d+)', file)
            if not cid:
                continue
            cid = cid.group(1)
...
def main() -> None:
    ...
    # Create a thread for each configuration for speedup
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        parsers = [Parser(join(input_dir, config_dir)) for config_dir in configuration_dirs]
        futures = [executor.submit(parser.process) for parser in parsers]
