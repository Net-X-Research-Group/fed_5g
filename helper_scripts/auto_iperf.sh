...
read -p "Start uplink test on OAIBOX. Press Enter to continue."
...
for ((CID=1;CID<=NUM_CLIENTS;CID++)); do
    ...
    read -p "Start downlink test. TCP, 100 Gbps, ip ${wwan}. Press Enter to continue."
    ssh "$LOGIN" iperf -s -i 1 -fm -B "$wwan" -t 30 > "iperf/${BW}_${TDD}_pi0${CID}_DL.txt"
    ...
    ssh "$LOGIN" iperf -t 30 -i 1 -fm -b 1000M -c 172.31.0.135 -B "$wwan" > "iperf/${BW}_${TDD}_pi0${CID}_UL.txt"
